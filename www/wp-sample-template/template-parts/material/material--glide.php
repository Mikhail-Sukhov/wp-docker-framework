...
if ( ! function_exists( 'wpsampletemplate_rebuild_theme_post_material' ) ) {
	function wpsampletemplate_rebuild_theme_post_material($classes) {
...
add_filter('post_class', 'wpsampletemplate_rebuild_theme_post_material');
			...
			<div class="glide__track" data-glide-el="track">
				<ul class="glide__slides material__slides">
								...
								<section class="post__content--material">
									...
									<?php endif; ?>
										...
										<?php if ( ( is_home() || is_archive() ) && ( ! is_sticky() ) ) : //Если Главная или Архив ?>
											...
											<a href="<?php the_permalink(); // Ссылка на пост ?>" class="post__readmore"><?php echo esc_html__( 'Readmore...', 'wpsampletemplate' ); ?></a>
					...
					<?php endwhile; else : //Конец цикла ?>
						...
						<li class="glide__slide">
							<article <?php post_class() ?> id="post-<?php the_ID(); ?>" data-post-id="<?php the_ID(); ?>">
								<h2><?php echo esc_html__( 'There are no posts', 'wpsampletemplate' ); ?></h2>
							</article>
						</li>
