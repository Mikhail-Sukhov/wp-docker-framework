...
if ( ! empty( get_post()->post_password ) ) {
	if ( $_COOKIE[ 'wp-postpass_' . COOKIEHASH ] != get_post()->post_password ) {
		...
		<p class="nocomments"><?php echo esc_html__( 'Enter the password', 'wpsampletemplate' ); ?></p>
	...
	<section class="m-comment" id="comments">
		...
		<?php if ( $comments ) : // Если есть комментарии ?>
			<div class="m-comment__list">
				<h2 class="m-comment__list-title"><?php echo esc_html__( 'Comments', 'wpsampletemplate' ); ?></h2>
		...
		<div class="m-comment__form">
			...
			<?php if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) : // Если незарегистрированным запрещено комментировать ?>
				...
				<div class="m-comment__err-mes">
					<p id="respond"><?php echo esc_html__( 'Please', 'wpsampletemplate' ); ?>, <a href="<?php echo get_option( 'siteurl' ); ?>/wp-login.php?redirect_to=<?php echo urlencode( get_permalink() ); ?>"><?php echo esc_html__( 'register', 'wpsampletemplate' ); ?></a> <?php echo esc_html__( 'to comment.', 'wpsampletemplate' ); ?></p>
				</div>

			<?php else : // Если нет (Если незарегистрированным запрещено комментировать) ?>

				<h2 class="m-comment__form-title" id="respond"><?php echo esc_html__( 'Post a comment', 'wpsampletemplate' ); ?></h2>
				...
				<form action="<?php echo get_option( 'siteurl' ); ?>/wp-comments-post.php" method="post" class="form">
					...
					<?php if ( is_user_logged_in() ) : // Если пользователь зарегистрирован ?>
						...
						<div class="m-comment__hello-mes">
							<p><?php echo esc_html__( 'Hello', 'wpsampletemplate' ); ?>, <a href="<?php echo get_option( 'siteurl' ); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo wp_logout_url( get_permalink() ); ?>" title="Выйти">Выход &raquo;</a></p>
						</div>
...
<?php else : // Если нет (Если комментарии разрешены) ?>
	<section class="m-comment" id="comments">
		<div class="m-comment__err-mes">
			<p><?php echo esc_html__( 'Comments on this post are closed!', 'wpsampletemplate' ); ?></p>
		</div>
	</section>
<?php endif; ?>